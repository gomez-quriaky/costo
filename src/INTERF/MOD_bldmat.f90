...
!=====================================================================
! Declaration of the in/out arguments of BLDMATq
!=====================================================================
...
real(kind=8), dimension(:,:), allocatable    ::bd_test
real(kind=8), dimension(:), allocatable    ::h1_test
integer                                      :: count
...
allocate(bd_test(npar,npar))
allocate(h1_test(npar))
...
! Step 1: Prepare weighted COO values
do i = 1, nnz
     weighted_values(i) =(punvar(columnAD(i))) * val_ad_s(i)
end do
...
status = MKL_SPARSE_CONVERT_CSR(Aderi_coo_t,SPARSE_OPERATION_NON_TRANSPOSE, Aderi_w_csr)
...
status = mkl_sparse_d_spmmd (SPARSE_OPERATION_TRANSPOSE, Aderi_w_csr, Aderi_csr,&
                    SPARSE_LAYOUT_ROW_MAJOR, bderi, npar)
print *, 'result  mm dense bderi', status

!  do i = 1, npar
     !  do j = 1, i - 1
       !bderi(j, i) = bderi(i, j)
          !  bd_test(j,i) = bd_test(i,j)
     !  end do
!   end do
! 
     ...
     ! do i=1,npar
          ! do j=1, npar
               ! if(bderi(i,j) .ne. 0 .and. bd_test(i,j).ne. bderi(i,j)) then
                    ! count = count +1
                    ! print *, "diff on bderi", bd_test(i,j) - bderi(i,j), "bderi", bderi(i,j), "b_test", bd_test(i,j)
               ! end if
          ! end do
     ! end do

     print *, "total diff", count
! !=====================================================================
...
! count =0
! do i=1,npar
     ! if(h1(i).ne. h1_test(i)) then
          ! count = count +1
          ! print *, "h1 diff", h1(i) - h1_test(i)
     ! end if
! end do 
